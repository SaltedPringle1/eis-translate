...
def translator_ui(mode_key, eng_func, target_func, target_name):
    ...
    with col1:
        source_label = "English" if st.session_state[mode_key] == f"English to {target_name}" else target_name.capitalize()
        # Input box keeps its key so it can register what you type
        input_text = st.text_area(f"{source_label} Input", height=250, placeholder=f"Enter {source_label} text...", key=f"input_{mode_key}")
    ...
    with col3:
        target_label = target_name.capitalize() if st.session_state[mode_key] == f"English to {target_name}" else "English"
        ...
        # BUG PATCHED HERE: Removed the 'key' argument and made the label unique.
        # This forces Streamlit to constantly refresh this box with the new translation!
        st.text_area(f"{target_label} Output ({target_name})", value=result, height=250)
